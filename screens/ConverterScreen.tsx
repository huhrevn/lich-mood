                        ...
                        <div className="bg-white dark:bg-zinc-900 rounded-[24px] shadow-sm border border-gray-100 dark:border-zinc-800 overflow-visible relative flex flex-col xl:p-8">
                            ...
                            <div className="hidden xl:flex justify-center mb-8">
                                <div className="bg-gray-100 dark:bg-zinc-800 p-1.5 rounded-full flex w-[400px] shadow-inner relative">
                                    ...
                                    <div 
                                        className={`absolute top-1.5 bottom-1.5 w-[calc(50%-6px)] bg-white dark:bg-zinc-700 rounded-full shadow-sm transition-all duration-300 ease-in-out z-0 ${
                                            state.mode === 'SOLAR_TO_LUNAR' ? 'left-1.5' : 'left-[calc(50%+3px)]'
                                        }`}
                                    ></div>

                                    <button
                                        onClick={() => actions.setMode('SOLAR_TO_LUNAR')}
                                        className={`relative z-10 flex-1 py-2 text-xs font-bold rounded-full transition-all duration-200 flex items-center justify-center gap-2 ${
                                            state.mode === 'SOLAR_TO_LUNAR' 
                                            ? 'text-gray-800 dark:text-zinc-100' 
                                            : 'text-gray-500 hover:text-gray-700 dark:text-zinc-400'
                                        }`}
                                    >
                                        <span className={`material-symbols-outlined text-[18px] ${state.mode === 'SOLAR_TO_LUNAR' ? 'text-orange-500' : ''}`}>wb_sunny</span>
                                        Dương → Âm
                                    </button>
                                    <button
                                        onClick={() => actions.setMode('LUNAR_TO_SOLAR')}
                                        className={`relative z-10 flex-1 py-2 text-xs font-bold rounded-full transition-all duration-200 flex items-center justify-center gap-2 ${
                                            state.mode === 'LUNAR_TO_SOLAR' 
                                            ? 'text-gray-800 dark:text-zinc-100' 
                                            : 'text-gray-500 hover:text-gray-700 dark:text-zinc-400'
                                        }`}
                                    >
                                        <span className={`material-symbols-outlined text-[18px] ${state.mode === 'LUNAR_TO_SOLAR' ? 'text-indigo-500' : ''}`}>dark_mode</span>
                                        Âm → Dương
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col xl:flex-row gap-6 xl:items-stretch">
                                ...
                                <div className="p-5 pb-0 xl:p-0 xl:w-[45%] flex flex-col">
                                    <ConverterForm 
                                        mode={state.mode}
                                ...
                                <div className="hidden xl:flex flex-col justify-center items-center w-[10%]">
                                    <button 
                                        ...
                                        className="size-12 rounded-full border border-gray-100 shadow-sm bg-gray-50 hover:bg-white hover:border-[#4A7B4F] hover:text-[#4A7B4F] text-gray-400 flex items-center justify-center transition-all group"
                                    >
                                ...
                                {state.result && !state.error && (
                                    <div className="px-5 pb-8 xl:p-0 xl:w-[45%] animate-[fadeIn_0.3s_ease-out] flex flex-col items-center justify-center border-t xl:border-t-0 xl:border-l border-gray-100 xl:pl-6 border-dashed">
                                        <div className="w-full h-full flex flex-col">
                                            {/* Result Header */}
                                            <div className="hidden xl:flex items-center gap-2 mb-6 text-xs font-bold text-[#4A7B4F] uppercase tracking-wide">
                        ...
                        {state.result && (
                            <>
                                ...
                                <div className="bg-white dark:bg-zinc-900 rounded-[24px] shadow-sm border border-gray-100 dark:border-zinc-800 p-6 animate-[fadeIn_0.4s_ease-out]">
                                    <div className="flex items-center gap-2 mb-4 text-[#4A7B4F]">
